...
<Note>
  When using `toolChoice: { type: 'tool', toolName: 'toolName' }` to force a specific tool,
  the model will call that tool but may not generate a text response afterward. If you want
  the model to both use tools and generate a response, use `toolChoice: 'auto'` (default) or
  `toolChoice: 'required'` instead, and let the model decide when to use tools based on your
